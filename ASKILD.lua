...
Server_DevASKILD = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_DevASKILD.."Token_DevASKILD") then
...
redis:set(Server_DevASKILD.."Token_DevASKILD",token)
end 
else
...
------------------------------------------------------------------------------------------------------------
if not redis:get(Server_DevASKILD.."User_DevASKILD1") then
...
------------------------------------------------------------------------------------------------------------
local DevASKILD_Info_Sudo = io.open("Info_Sudo.lua", 'w')
DevASKILD_Info_Sudo:write([[
do 
local ASKILD_INFO = {
Id_DevASKILD = ]]..redis:get(Server_DevASKILD.."Id_DevASKILD")..[[,
UserName_ASKILD = "]]..redis:get(Server_DevASKILD.."User_DevASKILD1")..[[",
Token_Bot = "]]..redis:get(Server_DevASKILD.."Token_DevASKILD")..[["
}
return ASKILD_INFO
end

]])
DevASKILD_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_ASKILD = io.open("ASKILD", 'w')
Run_File_ASKILD:write([[
#!/usr/bin/env bash
cd $HOME/ASKILD
token="]]..redis:get(Server_DevASKILD.."Token_DevASKILD")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./ASKILD.lua -p PROFILE --bot=$token
done
]])
Run_File_ASKILD:close()
------------------------------------------------------------------------------------------------------------
...
while(true) do
rm -fr ../.telegram-cli
screen -S DevASKILD -X kill
screen -S DevASKILD ./ASKILD
done
]])
Run_SM:close()
...
os.execute('chmod +x ASKILD')
os.execute('./Run')
Status = true
else   
f:close()  
redis:del(Server_DevASKILD.."Token_DevASKILD");redis:del(Server_DevASKILD.."Id_DevASKILD");redis:del(Server_DevASKILD.."User_DevASKILD1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_ASKILD
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DevASKILD
Ids_Dev = {sudos.Id_DevASKILD}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "ASKILD"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_ASKILD(msg)  
local Dev_ASKILD = false  
...
function Dev_ASKILD_User(user)  
local Dev_ASKILD_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_ASKILD_User = true  
end  
end  
return Dev_ASKILD_User  
end 
function DeveloperBot(msg) 
local Status = redis:sismember(bot_id.."Developer:Bot", msg.sender_user_id_) 
if Status or Dev_ASKILD(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_ASKILD(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_ASKILD(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_ASKILD(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_ASKILD(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_ASKILD(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
if Dev_ASKILD_User(user_id) then
Status = true  
...
if Dev_ASKILD_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/ASKILDTEAM/ASKILD/master/Script.lua'
...
url = 'https://raw.githubusercontent.com/ASKILDTEAM/ASKILD/master/ASKILD.lua'
file_path = 'ASKILD.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
options.redirect = false 
...
local UserName = (data.username_ or "ASKILDcli")
...
------------------------------------------------------------------------------------------------------------
function FilesASKILD(msg)
File_Bot = dofile("Script.lua")
if File_Bot.ASKILD and msg then
Text_File = File_Bot.ASKILD(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesASKILDBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.ASKILDFile and msg then
Text_FileBot = Text_FileBot.ASKILDFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_ASKILD_File(msg,data)
if msg then
msg = data.message_
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:DevASKILD'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:DevASKILD'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'〽┇تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:DevASKILD',text)
redis:del(bot_id..'GetTexting:DevASKILD'..msg.chat_id_..':'..msg.sender_user_id_)
...
download_to_file('https://raw.githubusercontent.com/ASKILDTEAM/ASKILD/master/ASKILD.lua','ASKILD.lua') 
download_to_file('https://raw.githubusercontent.com/ASKILDTEAM/ASKILD/master/Script.lua','Script.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/ASKILDTEAM/ASKILDFile/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n📦┇قائمه ملفات متجر سورس ASKILD\n💢┇الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/ASKILDTEAM/ASKILDFile/master/plugins_/"..File_Name)
if Res == 200 then
...
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_ASKILD(msg) then
